...
def main(target_model_path):

    seed_all(60065)
    Path("model_inversion/models").mkdir(parents = True, exist_ok = True)
    Path("model_inversion/inverted_faces").mkdir(parents = True, exist_ok = True)
    ...
    training_image_loader = ImageLoader(data_paths = ["model_inversion/faces/training", "model_inversion/faces/testing"], num_of_training_images = 9, num_of_labels = 40)
    ...
    if not target_model_path:
        ...
        save_model(target, 'model_inversion/models/mi_target_model.pt')
        plot_losses(train_losses, 'model_inversion/loss.png')
    else:
    ...
    plot_Inverted_face(img, f'model_inversion/inverted_faces/21_loss_{loss:.9f}.png', 21, loss)

    # inverting class 30
    print('inverting class #30')
    img, loss = MI_FACE(target, 30, (1, 1, IMG_SIZE, IMG_SIZE), 100, 1000, normalize_weight)
    plot_Inverted_face(img, f'model_inversion/inverted_faces/30_loss_{loss:.9f}.png', 30, loss)

    # inverting class 32
    print('inverting class #32')
    img, loss = MI_FACE(target, 32, (1, 1, IMG_SIZE, IMG_SIZE), 100, 1000, normalize_weight)
    plot_Inverted_face(img, f'model_inversion/inverted_faces/32_loss_{loss:.9f}.png', 32, loss)

    # inverting class 33
    print('inverting class #33')
    img, loss = MI_FACE(target, 33, (1, 1, IMG_SIZE, IMG_SIZE), 100, 1000, normalize_weight)
    plot_Inverted_face(img, f'model_inversion/inverted_faces/33_loss_{loss:.9f}.png', 33, loss)

    # inverting class 38
    print('inverting class #38')
    img, loss = MI_FACE(target, 38, (1, 1, IMG_SIZE, IMG_SIZE), 100, 1000, normalize_weight)
    plot_Inverted_face(img, f'model_inversion/inverted_faces/38_loss_{loss:.9f}.png', 38, loss)
